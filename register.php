...
$username_err = $password_err = $email_err = $phone_err = $user_type_err = "";
$registration_msg = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($username_err) && empty($password_err) && empty($email_err) && empty($phone_err) && empty($user_type_err)) {
        // Insert query
        $sql = "INSERT INTO users (username, password, email, phone, user_type) VALUES (?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "sssss", $param_username, $param_password, $param_email, $param_phone, $param_user_type);
        ...
        if (mysqli_stmt_execute($stmt)) {
            $registration_msg = '<div class="alert alert-success">Registration successful!</div>';
        } else {
                ...
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <select name="user_type" class="form-control <?php echo (!empty($user_type_err)) ? 'is-invalid' : ''; ?>">
                            ...
                            <option value="organizer" <?php echo ($user_type == 'organizer') ? 'selected' : ''; ?>>Organizer</option>
                            <option value="attendee" <?php echo ($user_type == 'attendee') ? 'selected' : ''; ?>>Attendee</option>
                        </select>
